...
struct Sphere:Hittable{
    ...
    float sdistance(const vf4d& p) override{
        ...
        p1.y = (p2.y + p2.z)*c;
        p1.z = -(p2.y - p2.z)*c;
        // return (std::abs(pp.x)+std::abs(pp.y)+std::abs(pp.z)+std::abs(pp.t) - 4*radius);
        return std::max(std::max(std::abs(p1.x),std::abs(p1.y)), std::max(std::abs(p1.z),std::abs(p1.t))) - radius;
...
public:
    ...
    bool OnUserUpdate(float fElapsedTime) override
    {
        ...
        auto seg = Segment{vf4d{x, y, z, t}, vf4d{1, 0, 0, 0}, 1};
...
int main()
{
    ...
    Example demo;
    if (demo.Construct(800, 600, 1, 1, 0, 0)){
